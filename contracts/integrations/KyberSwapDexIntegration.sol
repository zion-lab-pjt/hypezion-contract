    ...
    UUPSUpgradeable
{
    ...
    // Authorized YieldManager address (for kHYPE → HYPE swaps during yield harvesting)
     ...
     * @dev Consolidates common logic for HYPE → kHYPE and External Token → kHYPE swaps
    ...
    function _swapToKHypeInternal(
        ...
        // 1. Execute swap inputToken → kHYPE via KyberSwap
     ...
     * @dev This replaces the old flow of: HYPE → stake via Kinetiq → kHYPE
     *      New flow is direct: HYPE → kHYPE via KyberSwap DEX
     ...
     * @param swapData Encoded swap data from KyberSwap API (HYPE → kHYPE)
     * @param minKHypeOut Minimum kHYPE to receive (slippage protection)
     * @return kHypeReceived Amount of kHYPE received after fee
     */
     ...
     * @dev For external tokens (USDC, USDT, etc.) → kHYPE
     ...
     * @param swapData Encoded swap data from KyberSwap API (inputToken → kHYPE)
     * @param minKHypeOut Minimum kHYPE to receive (slippage protection)
     * @return kHypeReceived Amount of kHYPE received after fee
     */
     ...
     * @param hypeToExternalSwapData Encoded swap data for HYPE → external token from KyberSwap API
     ...
     *      Step 1: Call this function to swap external token → HYPE (user receives HYPE)
     ...
     * @param swapData Encoded swap data from KyberSwap API (externalToken → HYPE)
