...
contract KinetiqYieldManager is AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    // Router address for secondary yield harvesting (consumes 1 slot; gap reduced 46 → 45)
    ...
    event SecondaryYieldHarvested(uint256 hypeReceived, uint256 timestamp);
    ...
    /**
     * @notice Harvest yield (Step 1): Swap kHYPE → HYPE and store for later compound
     * @dev Restricted to OPERATOR_ROLE.
     *      Flow: Withdraw kHYPE from vault → Swap kHYPE → HYPE via DEX → Store HYPE
     *      Call compound() separately with fresh swap data for exact HYPE amount.
     *
     * @param kHypeToHypeSwapData Encoded swap data from KyberSwap API (kHYPE → HYPE)
    ...
    function harvest(
        ...
        // 6. Swap kHYPE → HYPE via DexIntegration
     ...
     * @dev Restricted to OPERATOR_ROLE.
     *      Flow: Use stored HYPE → Mint hzUSD → Compound to StabilityPool
     *      Requires harvest() to be called first.
     *
     * @param hypeToKHypeSwapData Encoded swap data from KyberSwap API (HYPE → kHYPE for minting)
    ...
    /**
     * @notice Set the Router address for secondary yield harvesting
     * @param _router Address of HypeZionExchangeRouter
     */
    ...
    /**
     * @notice Harvest yield from Valantis secondary sources (Step 1b).
     * @dev Callable by OPERATOR_ROLE. Delegates to Router.harvestSecondaryYield() which
     *      uses a full-exit + re-deposit pattern to crystallise LP yield.
     *      Harvested HYPE is added to pendingHarvestedHype so the existing compound()
     *      step picks it up automatically — no additional compound call is needed.
     *
     * @param minYield Minimum total HYPE yield required. Pass 0 to harvest any dust-free yield.
     * @return hypeReceived HYPE added to pendingHarvestedHype
     */
    function harvestSecondary(uint256 minYield)
        external nonReentrant onlyRole(OPERATOR_ROLE) returns (uint256 hypeReceived)
    {
        if (router == address(0)) revert InvalidAddress();

        hypeReceived = IHypeZionExchangeRouter(router).harvestSecondaryYield(minYield);

        if (hypeReceived == 0) revert NoYieldToHarvest();

        pendingHarvestedHype += hypeReceived;
        lastHarvestTimestamp = block.timestamp;

        emit SecondaryYieldHarvested(hypeReceived, block.timestamp);
    }
