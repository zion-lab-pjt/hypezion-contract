...
interface IHypeZionExchange {
    ...
    event StablecoinMinted(
        ...
        uint256 hypAmount,
        uint256 zusdMinted,
        uint256 usdValueInvested,
        uint256 feeCharged
    );
    event LevercoinMinted(
        address indexed user,
        uint256 hypAmount,
        uint256 zhypeMinted,
        uint256 usdValueInvested,
    ...
    event SwapStableToLever(
        address indexed user,
        uint256 zusdAmount,
        uint256 zhypeReceived,
        uint256 usdValueSwapped
    );
    event SwapLeverToStable(
        address indexed user,
        uint256 zhypeAmount,
        uint256 zusdReceived,
        uint256 usdValueSwapped
    );
    ...
    event SwapMintFeeUpdated(uint256 newFeeBps);
    event YieldSettlementQueued(
        uint256 indexed withdrawalId,
        uint256 yieldAmount
    );
    event TokenFeesUpdated(
        uint16[3] bullHypeMintFees,
        uint16[3] bullHypeRedeemFees,
        uint16[3] hzUsdMintFees,
        uint16[3] hzUsdRedeemFees
    );
    event InterventionManagerUpdated(
    ...
    ) external payable returns (uint256 zusdMinted);

    function mintLevercoin(
        uint256 amountHYPE,
        bytes calldata swapData
    ) external payable returns (uint256 zhypeMinted);
