...
/// @dev Minimal interface for Exchange v2 fields not yet in IHypeZionExchange.
///      Used with try-catch so this contract remains compatible with Exchange v1.
interface IHypeZionExchangeV2 {
    function totalSecondaryCollateral() external view returns (uint256);
    function router() external view returns (address);
}

/// @dev Minimal interface for Router — used to query live adapter cost basis and reserves.
interface IHypeZionRouter {
    function getTotalSecondaryDeposited() external view returns (uint256);
...
contract HypeZionExchangeInformation is AccessControlUpgradeable, UUPSUpgradeable {
    ...
    // Combined rate snapshots: week => (totalReserveInHYPE / totalCollateral) * 1e18
    // Captures yield from ALL sources (Kinetiq + Valantis). Populated by updateRateSnapshot().
    // Falls back to rateSnapshots during the first 2 weeks after upgrade (warm-up period).
    mapping(uint256 => uint256) public combinedRateSnapshots;

    // Storage gap for future upgrades (UUPS pattern)
    // Reduced from 46 to 45 after adding combinedRateSnapshots
     ...
     * @return rate Combined rate (1e18 = 1:1). Grows as Kinetiq AND secondary source yield accrues.
     * @dev combined_rate = totalReserveInHYPE / (totalHYPECollateral + totalSecondaryCollateral)
     *      Uses try-catch for Exchange v1 compatibility (totalSecondaryCollateral may not exist).
     *      Returns 0 on any failure so callers can fall back gracefully.
     */
    function _getCombinedRate() internal view returns (uint256 rate) {
        uint256 totalReserve;
        try exchange.getTotalReserveInHYPE() returns (uint256 r) {
            totalReserve = r;
        } catch {
            return 0;
        }

        uint256 primaryCollateral = exchange.totalHYPECollateral();

        // Prefer live adapter cost basis from Router (always accurate) over
        // Exchange.totalSecondaryCollateral (can become stale after harvest
        // re-deposit failures or adapter value losses).
        uint256 secondaryCollateral = _getSecondaryCollateral();

        uint256 totalCollateral = primaryCollateral + secondaryCollateral;
        if (totalCollateral == 0) return 0;

        return (totalReserve * PRECISION) / totalCollateral;
    }

    /// @dev Get secondary collateral: prefer live Router data, fall back to Exchange cost basis.
    function _getSecondaryCollateral() internal view returns (uint256) {
        // Try Router.getTotalSecondaryDeposited() — always accurate
        try IHypeZionExchangeV2(address(exchange)).router() returns (address routerAddr) {
            if (routerAddr != address(0)) {
                try IHypeZionRouter(routerAddr).getTotalSecondaryDeposited() returns (uint256 sd) {
                    return sd;
                } catch {}
            }
        } catch {}

        // Fallback: Exchange.totalSecondaryCollateral (may be stale)
        try IHypeZionExchangeV2(address(exchange)).totalSecondaryCollateral() returns (uint256 sc) {
            return sc;
        } catch {
            return 0;
        }
    ...
    function updateRateSnapshot() external {
        ...
        if (currentWeek > latestSnapshotWeek) {
            ...
            // Save Kinetiq rate snapshot (legacy, kept for fallback during warm-up)
            rateSnapshots[currentWeek] = currentRate;

            // Save combined rate snapshot (Kinetiq + secondary sources)
            uint256 combinedRate = _getCombinedRate();
            if (combinedRate > 0) {
                combinedRateSnapshots[currentWeek] = combinedRate;
            }
            ...
            if (currentWeek > 10) {
                ...
                delete combinedRateSnapshots[currentWeek - 10];
            }
    ...
    function _calculateBaseKinetiqAPY(uint256 kinetiqExchangeRate) internal view returns (uint256 baseAPY) {
        ...
        // Prefer combined rate (Kinetiq + secondary sources) when available.
        // Falls back to legacy Kinetiq-only rate during the 2-week warm-up after upgrade.
        uint256 twoWeeksAgoCombinedRate = combinedRateSnapshots[twoWeeksAgoWeek];
        if (twoWeeksAgoCombinedRate > 0) {
            uint256 currentCombinedRate = _getCombinedRate();
            if (currentCombinedRate == 0 || currentCombinedRate <= twoWeeksAgoCombinedRate) {
                return 0;
            }
            uint256 combinedGrowthBps = ((currentCombinedRate - twoWeeksAgoCombinedRate) * BASIS_POINTS) / twoWeeksAgoCombinedRate;
            return (combinedGrowthBps * SECONDS_PER_YEAR) / APY_WINDOW;
        }

        // Fallback: use legacy Kinetiq-only rate snapshots (pre-upgrade or warm-up period)
    ...
    struct DefiLlamaTVL {
        uint256 totalTVLInHYPE;          // Total TVL across all sources (18 decimals)
        uint256 primaryReserveInHYPE;    // kHYPE direct holdings via Kinetiq (18 decimals)
        uint256 secondaryReserveInHYPE;  // Valantis pools + stHYPE staking adapters (18 decimals)
        uint256 stakedHzUSDAmount;       // HzUSD deposited in shzUSD vault (18 decimals)
        uint256 hypePriceUSD;            // Current HYPE price in USD for conversion (18 decimals)
    }

    struct DefiLlamaYield {
        uint256 stabilityPoolAPY;   // shzUSD staking APY in basis points (10000 = 100%)
        uint256 totalStakedHzUSD;   // Total HzUSD staked in shzUSD vault (18 decimals)
        uint256 stakedTVLInUSD;     // shzUSD pool TVL in USD (18 decimals)
    }

    /**
     * @notice Get protocol TVL breakdown for DeFiLlama integration
     * @dev Single entry point for TVL adapter — aggregates all yield sources internally.
     *      DeFiLlama adapter only needs to call this one function.
     * @return tvl DefiLlamaTVL struct with complete TVL breakdown
     */
    function getProtocolTVL() external view returns (DefiLlamaTVL memory tvl) {
        // Total reserve (primary + secondary) — canonical source of truth
        try exchange.getTotalReserveInHYPE() returns (uint256 totalReserve) {
            tvl.totalTVLInHYPE = totalReserve;
        } catch {
            tvl.totalTVLInHYPE = exchange.totalHYPECollateral();
        }

        // Secondary reserve from Router (Valantis pools + stHYPE staking)
        tvl.secondaryReserveInHYPE = _getSecondaryReserve();

        // Primary = total - secondary (kHYPE via Kinetiq)
        tvl.primaryReserveInHYPE = tvl.totalTVLInHYPE > tvl.secondaryReserveInHYPE
            ? tvl.totalTVLInHYPE - tvl.secondaryReserveInHYPE
            : 0;

        // shzUSD vault (HzUSD staked — separate from main TVL)
        try IStabilityPool(exchange.stabilityPool()).totalAssets() returns (uint256 assets) {
            tvl.stakedHzUSDAmount = assets;
        } catch {
            tvl.stakedHzUSDAmount = 0;
        }

        // HYPE price for USD conversion
        try IOracle(exchange.oracle()).getPrice("HYPE") returns (IOracle.PriceData memory priceData) {
            tvl.hypePriceUSD = priceData.price;
        } catch {
            tvl.hypePriceUSD = 0;
        }
    }
    ...
    /**
     * @notice Get protocol yield information for DeFiLlama integration
     * @dev Single entry point for yield adapter — returns APY and TVL for yield pools.
     *      Currently exposes the shzUSD stability pool yield.
     * @return yield DefiLlamaYield struct with yield pool metrics
     */
    function getProtocolYield() external view returns (DefiLlamaYield memory yield) {
        IStabilityPool stabilityPool = IStabilityPool(exchange.stabilityPool());
        HzUSD zusd = HzUSD(exchange.zusd());
        uint256 zusdSupply = zusd.totalSupply();

        // shzUSD staking APY (14-day rolling window)
        IKinetiqIntegration kinetiq = IKinetiqIntegration(exchange.kinetiq());
        uint256 kinetiqExchangeRate = PRECISION;
        try kinetiq.getExchangeRate() returns (uint256 rate) {
            kinetiqExchangeRate = rate;
        } catch {}

        try this.calculateStabilityPoolAPYSafe(stabilityPool, kinetiqExchangeRate, zusdSupply) returns (uint256 apy) {
            yield.stabilityPoolAPY = apy;
        } catch {
            yield.stabilityPoolAPY = 0;
        }

        // Total HzUSD staked
        try stabilityPool.totalAssets() returns (uint256 assets) {
            yield.totalStakedHzUSD = assets;
        } catch {
            yield.totalStakedHzUSD = 0;
        }

        // TVL in USD: stakedHzUSD is denominated in HzUSD which is pegged to $1
        // So TVL in USD ≈ totalStakedHzUSD (already in 18 decimals = $1 per unit)
        yield.stakedTVLInUSD = yield.totalStakedHzUSD;
    }

    /// @dev Get total secondary reserve from Router. Returns 0 on any failure.
    function _getSecondaryReserve() internal view returns (uint256) {
        try IHypeZionExchangeV2(address(exchange)).router() returns (address routerAddr) {
            if (routerAddr != address(0)) {
                try IHypeZionRouter(routerAddr).getTotalSecondaryReserveInHYPE() returns (uint256 sr) {
                    return sr;
                } catch {}
            }
        } catch {}
     ...
     * @dev Required when Exchange proxy address changes (e.g., redeployment)
